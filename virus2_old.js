...
function createMesh() {
			...
			for ( i = 0; i < vl; i ++ ) {
				...
				if (i < Math.min(pmesh.geometry.vertices.length, vl)) {
					...
					kx[i] =  (vertices[i].x - pmesh.geometry.vertices[i].x)/n;
					ky[i] =  (vertices[i].y - pmesh.geometry.vertices[i].y)/n;
					kz[i] =  (vertices[i].z - pmesh.geometry.vertices[i].z)/n;
			...
			particles.scale.x = particles.scale.y = particles.scale.z = 6;
			scene.add( particles );
...
function applyAnim() {
	...
	$('.navi').click(function() {
		...
		for (var i = pmesh.geometry.vertices.length - 1; i >= 0; i--) {
			vel[i] = Math.random()*speed+speed/2;
		}
		console.log('MESH: OK');
	...
	requestAnimationFrame( animate );
		...
		if (dt < n) {
			...
			for (var i = pmesh.geometry.vertices.length - 1; i >= 0; i--) {
				pmesh.geometry.vertices[i].x += Math.sin(dt/15);
		...
		for (var i = tmp.geometry.vertices.length - 1; i >= 0; i--) {
			if (tmp.geometry.vertices[i].y > 0){
